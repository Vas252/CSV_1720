...
def get_data_from_file(
        ...
        flag_record, time_start_date, fag_file, search_kks):
    # flag_record - 0 или 1,
    ...
    for count in range(left_edge, right_edge):
        ...
        # Прроверим содержит ли строка нужный нам KKS
        if test.count(search_kks) or search_kks == '0':
            # Если строка то, что нам нужно разобъем ее на части по знаку \t
            test_str = test.rsplit(sep='\t')
            # И возьмем из нее дату и значение
            date_time_event_test = test_str[0]
            if search_kks == '0':
                data_event_test = test_str[1]
            else:
                data_event_test = test_str[3].rstrip()
            ...
            if flag_record == 1:
                if fag_file == 0:
                    ...
                    if float(data_event_test) < 10:
                        ...
                        data_event = float(data_event_test)
...
def read_files_txt_scalar(dir_name, file_name, search_kks):
    ...
    try:
        ...
        result = get_data_from_file(
            0, len(temp_file_content), temp_file_content, 1, 0, 0, search_kks)
    finally:
        f.close()
    return result


def create_parse_file(
        file_dir, file_name_source,
        file_name_receiver, search_kks):
    file_dir_name = file_dir
    data_source_file = read_files_txt_scalar(
        file_dir_name, file_name_source, search_kks)
...
def create_normal_file(
    ...
    search_kks = input_data.put_kks()
    print(f'Искомый KKS - {search_kks}')
    ...
    try:
        ...
        # 3. Нули, файл, обрезка файла
            ...
            right_edge = borders_arr[1]
            result_middle = get_data_from_file(
                0, right_edge, temp_file_content, 1, 0, 1, '0')
        ...
        # 5. Файл (времена равны)
            ...
            # variant = 5
            result = get_data_from_file(
                0, len(temp_file_content), temp_file_content, 1, 0, 1, '0')
    finally:
        f.close()
    return result
